...
class ScriptUploadView(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        script, created = models.Script.objects.get_or_create(name=form.cleaned_data['name'])
        if script.versions.count() == 0:
            models.ScriptVersion.objects.create(version=form.cleaned_data['version'], content=json_loaded, script=script)
        elif Version(form.cleaned_data['version']) < Version(str(script.latest_version().version)):
            print("Bad Version")
        else:
            models.ScriptVersion.objects.create(version=form.cleaned_data['version'], content=json_loaded, script=script)
        return HttpResponse()
